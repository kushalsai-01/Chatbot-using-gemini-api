...
export default function ChatWindow({ messages, isLoading, onSuggestionClick, darkMode }) {
  ...
  if (messages.length === 0 && !isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 select-none relative overflow-hidden">
        ...
        <div className="absolute w-[500px] h-[500px] rounded-full bg-[var(--spotlight)] blur-[100px] pointer-events-none" />

        <div className="relative z-10 flex flex-col items-center gap-8 max-w-md w-full">
          {/* Icon */}
          <div className="relative animate-fade-up">
            <div className="w-16 h-16 rounded-2xl bg-[var(--icon-bg)] flex items-center justify-center shadow-[0_0_60px_-15px_rgba(255,255,255,0.15)]">
              <BsStars className="w-7 h-7 text-[var(--icon-text)]" />
            </div>
            <div className="absolute -inset-4 rounded-3xl border border-[var(--border)] animate-icon-spin" style={{ animationDuration: "20s" }} />
          </div>
          ...
          >
            {SUGGESTIONS.map(({ icon: Icon, label, desc }) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-[var(--accent-dim)] flex items-center justify-center flex-shrink-0 group-hover:bg-[var(--bg-hover)] transition-colors">
  ...
  return (
    <div className="flex-1 overflow-y-auto py-6">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 flex flex-col gap-1">
        {messages.map((msg, i) => (
          <MessageBubble key={i} role={msg.role} content={msg.content} index={i} darkMode={darkMode} />
        ))}
